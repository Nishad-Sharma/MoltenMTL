...
MoltenMTL is a Swift library that lets you write GPU code against Apple's **Metal API** — `MTLDevice`, `MTLCommandBuffer`, `MTLTexture`, and friends — and have it compile and run on **Vulkan**. Think of it as the inverse of [MoltenVK](https://github.com/KhronosGroup/MoltenVK): where MoltenVK translates Vulkan into Metal on Apple hardware, MoltenMTL translates the Metal API surface into Vulkan on Windows (and eventually Linux).
