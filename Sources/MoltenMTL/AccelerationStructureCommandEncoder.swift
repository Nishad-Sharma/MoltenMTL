...
public final class MTLAccelerationStructureCommandEncoder {
    ...
    /// Finishes encoding. No Vulkan call is required; temporary build buffers are released in `deinit`.
                           ...
                           vkDev:                 VkDevice)
    {
        ...
        for geom in geoms {
            guard let vbHandle = geom.vertexBuffer?.handle,
                ...
                print("[MoltenMTL] BLAS build: missing vertex or index buffer")
                return
            }
                           ...
                           vkDev:                 VkDevice)
    {
        ...
        guard instanceCount > 0,
            ...
            print("[MoltenMTL] TLAS build: instanceCount=0 or missing instance buffer")
            return
        }
        ...
        guard CVMA_createBuffer(vmaAlloc, instBufSize, instUsage,
            ...
            print("[MoltenMTL] TLAS build: instance buffer allocation failed")
            return
        }
        ...
        for i in 0..<instanceCount {
            ...
            if blasIdx < descriptor.instancedAccelerationStructures.count {
                ...
                print("[MoltenMTL] TLAS build: accelerationStructureIndex \(blasIdx) out of range")
                dst.blasAddress = 0
            }
