...
/// Records a sequence of GPU commands and submits them as a unit.
/// Obtain via `MTLCommandQueue.makeCommandBuffer()`. Recording begins immediately on creation.
/// Call `commit()` to submit, then `waitUntilCompleted()` to block until the GPU finishes.
public final class MTLCommandBuffer {
    ...
    public func commit() {
        ...
        guard vkEndCommandBuffer(handle) == VK_SUCCESS else {
            print("[MoltenMTL] vkEndCommandBuffer failed")
            return
        }
        ...
        guard vkCreateFence(dev, &fenceCI, nil, &f) == VK_SUCCESS else {
            print("[MoltenMTL] vkCreateFence failed")
            return
        }
        self.fence = f
        ...
        if let drawable = pendingDrawable {
            ...
            if result != VK_SUCCESS && result.rawValue != 1000001003 /* VK_SUBOPTIMAL_KHR */ {
                print("[MoltenMTL] CVKS_submitAndPresent failed (VkResult \(result.rawValue))")
            }
        } else {
            var h: VkCommandBuffer? = handle
            withUnsafePointer(to: &h) { cmdPtr in
                ...
                if vkQueueSubmit(queue, 1, &submitInfo, f) != VK_SUCCESS {
                    print("[MoltenMTL] vkQueueSubmit failed")
                }
            }
        }
    }

    /// Schedules `drawable` to be presented after `commit()`.
    /// Must be called before `commit()`. Submission will use swapchain semaphore synchronisation automatically.
    public func present(_ drawable: CAMetalDrawable) {
        pendingDrawable = drawable
    }

    /// Creates an encoder for buffer-to-image copy operations and image-to-image blits.
