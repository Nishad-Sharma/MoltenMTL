...
/// Mirrors MTLOrigin — a 3-D texel coordinate within a texture.
...
/// Mirrors MTLRegion — an axis-aligned rectangular region of a texture.
...
public final class MTLTexture {
    ...
    /// Mirrors `MTLTexture.heap`. Holding this reference keeps the heap alive.
    ...
    // MARK: - replace(region:mipmapLevel:withBytes:bytesPerRow:)

    /// Mirrors `MTLTexture.replace(region:mipmapLevel:withBytes:bytesPerRow:)`.
    ///
    /// Synchronously uploads CPU pixel data into the texture at `mipmapLevel`.
    ///
    /// Implementation:
    /// 1. Allocates a host-visible staging buffer via VMA and fills it from `bytes`.
    /// 2. Records a one-shot command buffer that performs:
    ///    - A pipeline barrier to transition the image into
    ///      `TRANSFER_DST_OPTIMAL`.
    ///    - A `vkCmdCopyBufferToImage` to copy the staging data.
    ///    - A pipeline barrier to transition the image into
    ///      `SHADER_READ_ONLY_OPTIMAL` (read-only) or `GENERAL` (storage).
    /// 3. Submits the command buffer and waits with `vkQueueWaitIdle`.
    public func replace(region:      MTLRegion,
        ...
        // pool is freed in the defer block above, which also frees cmdBuf.
    }
}

// MARK: - imageBarrier (module-internal helper)

/// Records a `VkImageMemoryBarrier` that transitions `image` from `oldLayout`
/// to `newLayout` with the specified access and pipeline stage masks.
