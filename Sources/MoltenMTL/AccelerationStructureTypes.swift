...
/// A tightly-packed three-component float vector with no alignment padding.
/// Used as the vertex format for acceleration structure geometry (`float3` in GLSL/HLSL).
...
/// Convenience constructor for `MTLPackedFloat3`, mirroring Metal's free function of the same name.
...
/// Memory requirements returned by `device.accelerationStructureSizes(descriptor:)`.
/// Use these sizes to allocate the acceleration structure and scratch buffers before building.
...
/// Common type for acceleration-structure build descriptors. Adopted by
/// `MTLPrimitiveAccelerationStructureDescriptor` (BLAS) and
/// `MTLInstanceAccelerationStructureDescriptor` (TLAS).
