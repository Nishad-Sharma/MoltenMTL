...
/// Controls where a resource is physically allocated.
public enum MTLStorageMode: Equatable {
    /// CPU + GPU accessible. The buffer is persistently host-mapped.
    case shared
    /// GPU-only (`DEVICE_LOCAL`). No CPU pointer is available.
    case `private`
}
...
public final class MTLBuffer {
    ...
    /// Returns the persistently-mapped CPU pointer for this buffer.
    /// Only valid for `.shared` buffers; calling this on a `.private` buffer will crash.
