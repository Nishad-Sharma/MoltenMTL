...
/// Records buffer-to-image copy and image-to-image blit commands.
/// Create via `commandBuffer.makeBlitCommandEncoder()`.
public final class MTLBlitCommandEncoder {
    ...
    ///   use `copy(from:sourceSlice:sourceLevel:sourceOrigin:sourceSize:to:...)` for full control.
    public func copy(from source: MTLTexture,
    ...
    /// - Note: **Not yet implemented.** Textures render correctly without mipmaps in the current pipeline.
    public func generateMipmaps(for texture: MTLTexture) {
        // TODO: implement per-level vkCmdBlitImage loop with barriers
    }

    /// Ends encoding. Commands are recorded directly into the parent command buffer, so no flush is needed.
